...
# a soma de todos os salários e a média dos salários. Todos os valores devem ser
# formatados para ter apenas duas casas decimais.
SELECT
	MAX(salary) as max,
    MIN(salary) as min,
    SUM(salary) as sum,
    ROUND(AVG(salary), 2) as med
FROM employees;
# 6- Escreva uma query que exiba a quantidade de pessoas que trabalham como pessoas programadoras (it_prog).
SELECT COUNT(job_id) FROM employees GROUP BY job_id HAVING job_id = 'it_prog';
# 7- Escreva uma query que exiba a quantidade de dinheiro necessária para efetuar o pagamento de cada profissão (job_id).
SELECT job_id, SUM(salary) as amount FROM employees GROUP BY job_id;
...
SELECT job_id, SUM(salary) as amount FROM employees GROUP BY job_id HAVING job_id = 'it_prog';
...
SELECT department_id, AVG(salary) as media FROM employees GROUP BY department_id HAVING COUNT(*) > 10;
